...
// const categoryNames = { 'us-news': 'U.S. News', opinion: 'Opinion', sport: 'Sport', business: 'Business', life: 'Life & Entertainment' };
// const categoryIntros = {
//   'us-news': 'Reporting that keeps you close to the decisions, people and places shaping our communities.',
...
const categoryNames = { 'us-news': 'U.S. News', world: 'World', sports: 'Sports', business: 'Business', finance: 'Finance' };
const categoryIntros = {
  'us-news': 'Reporting that keeps you close to the decisions, people and places shaping our communities.',
  world: 'Global news and analysis from around the world.',
  sports: 'The scores, stories and human moments behind every contest.',
  business: 'Clear reporting on enterprise, work and the changing economy.',
  finance: 'In-depth coverage of financial markets, economics and personal finance.',
};
const categoryHeroBg = {
  'us-news': 'bg-green',
  world: 'bg-[#343238]',
...
// Fallback image — make sure this file actually exists in /public/images
const FALLBACK_IMAGE = '/images/placeholder.jpg';

// Guards against undefined/null/empty/malformed image values, which is what
// causes next/image to throw "Failed to construct 'URL': Invalid URL".
function safeImageSrc(src) {
  if (!src || typeof src !== 'string' || src.trim() === '') return FALLBACK_IMAGE;
  if (src.startsWith('http://') || src.startsWith('https://') || src.startsWith('/')) {
    return src;
  }
  // relative path missing a leading slash — fix it rather than let it crash
  return `/${src}`;
}
...
export default async function CategoryPage({ params }) {
  ...
  return (
    <main className="pb-[60px]">
      ...
      />
      <header
        className={`relative overflow-hidden py-12 pb-[42px] text-white ${
          categoryHeroBg[category] ?? 'bg-green'
        } after:absolute after:-right-20 after:-top-[250px] after:h-[420px] after:w-[420px] after:rounded-full after:border after:border-white/18 after:content-[''] max-[760px]:py-[34px]`}
      >
        <div className="relative mx-auto w-[min(1180px,calc(100%-40px))]">
          <h1 className="my-[18px] mb-[15px] text-[clamp(56px,9vw,112px)] leading-[.86] tracking-[-.055em] max-[760px]:text-[58px]">
      ...
      <div className="mx-auto mt-[5px] w-[min(1180px,calc(100%-40px))]">
        <section className="grid min-h-[430px] grid-cols-[1.35fr_.65fr] bg-cream max-[760px]:block">
          <Link
            className="relative min-h-[430px] max-[760px]:block max-[760px]:aspect-[1.25] max-[760px]:min-h-0"
            href={`/${lead.category}/${lead.slug}`}
          >
            <Image
              src={safeImageSrc(lead.image)}
              alt={lead.title ?? ''}
              fill
              priority
          ...
          <div className="flex flex-col justify-center p-10 max-[760px]:p-6">
            <span className="mb-[7px] inline-block text-xs font-extrabold uppercase tracking-[.08em] text-green">
              Top story
            </span>
            <h2 className="my-1.5 mb-3.5 text-[clamp(32px,4vw,50px)] leading-[1.02] max-[760px]:text-[32px]">
              <Link href={`/${lead.category}/${lead.slug}`}>{lead.title}</Link>
        ...
        <div className="mt-[50px] grid grid-cols-[1fr_320px] gap-[60px] max-[1050px]:grid-cols-[1fr_280px] max-[1050px]:gap-[35px] max-[760px]:mt-[34px] max-[760px]:block">
          <section>
            <h2 className="m-0 border-b-2 border-green pb-2.5 text-[28px]">
              Latest in {categoryNames[category]}
            </h2>
            {rest.map((article) => (
              <StoryCard
                key={article.id}
                article={{ ...article, image: safeImageSrc(article.image) }}
                variant="feed"
              />
            ))}
          </section>
